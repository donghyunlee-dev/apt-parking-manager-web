...
import type { VisitHistory, VisitorVehicle } from './types';
import {
...
const VisitorPage = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['visitor-vehicles'] });
      pushToast({ type: 'success', message: '방문 차량을 삭제했습니다.' });
    },
    ...
    () => [
      ...
      },
      {
        ...
        cell: (row: VisitorVehicle) => (
          <button
            type="button"
            className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-600 dark:border-slate-700 dark:text-slate-200"
            onClick={(event) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6">
        <div>
          <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">방문 차량 관리</h2>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">방문 차량 등록과 방문 이력을 관리합니다.</p>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            />
            <div className="mt-3 text-sm text-slate-500 dark:text-slate-400">행 클릭: 수정 / 이력 버튼: 방문 이력</div>
          </>
        ) : (
          <EmptyState
            title="방문 차량이 없습니다."
            description="새로운 방문 차량을 등록해보세요."
            action={
              <button
                type="button"
              ...
              <button
                ...
                onClick={() => {
                  if (window.confirm('정말 삭제하시겠습니까?')) {
                    deleteMutation.mutate(editing.visitor_id);
                  }
                }}
              >
                삭제
        ...
        <div className="flex flex-col gap-3">
          {selectedVisitor && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-800 dark:bg-slate-900">
              차량번호: {selectedVisitor.vehicle_number} / 방문 호수:{' '}
              {selectedVisitor.building}동 {selectedVisitor.unit}호
            </div>
          )}
