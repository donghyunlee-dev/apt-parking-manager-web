...
This repository is currently minimal and contains only top-level metadata. Source code, tests, and assets have not been added yet. When code is introduced, keep it organized under a conventional layout such as `src/` for application code, `tests/` for automated tests, and `public/` or `assets/` for static files.

Current files:
- `package.json` for Node.js metadata and scripts.
- `README.md` for project overview.
- `AGENTS.md` for contributor guidance.

## Build, Test, and Development Commands
Defined scripts (see `package.json`):
- `npm test`: placeholder that prints “no tests yet”. Replace with a real test runner once tests exist.
- `npm run lint`: placeholder that prints “no lint yet”. Replace with a linter command when tooling is added.

If you add build or dev scripts (e.g., `npm run dev`, `npm run build`), document their purpose here with a short description and any required environment variables.

## Coding Style & Naming Conventions
No style rules are defined yet. When adding code, establish and document:
- Indentation (2 spaces is typical for JS/TS).
- File naming (e.g., `kebab-case.js` or `camelCase.ts`).
- Formatting/linting tools (e.g., ESLint, Prettier) and how to run them.

## Testing Guidelines
No test framework is configured. When tests are introduced, specify:
- Framework (e.g., Jest, Vitest).
- File naming conventions (e.g., `*.test.js` in `tests/` or alongside modules).
- Coverage expectations, if any, and how to run the suite.

## Commit & Pull Request Guidelines
Git history currently contains a single commit (“Initial commit”), so there is no established commit convention. Until a convention is defined, use clear, imperative messages like “Add login page” or “Refactor API client.”

For pull requests:
- Include a concise summary and list of changes.
- Link relevant issues when applicable.
- Add screenshots for UI changes.
- Note any new scripts, configs, or environment variables.

## Security & Configuration Tips
Do not commit secrets. If adding configuration, prefer `.env` files and document required variables in `README.md` with safe example values.
